...
public:
	...
	const Texture* getShadowMap(unsigned int index) const { 
		if(index >= MaxLightCount) return nullptr; 
		return &ShadowMaps[index]; 
	}

	Matrix getShadowMatrix(unsigned int index) const {
		if(index >= MaxLightCount) return Matrix(); 
		// Light Space Transform: Projection * View
		return ShadowCams[index].getProjectionMatrix() * ShadowCams[index].getViewMatrix();
	}
