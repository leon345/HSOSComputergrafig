...

{
    ...
    if (!loaded)
        throw std::exception("Phong kann nicht geladen werden.");
    assignLocations();
}
...
void PhongShader::assignLocations()
{
    ...
    OutlineColorLoc = glGetUniformLocation(ShaderProgram, "OutlineColor");
    OutlineWidthLoc = glGetUniformLocation(ShaderProgram, "OutlineWidth");
...
void PhongShader::activate(const BaseCamera& Cam) const
{
    ...
    if (UpdateState & OUTLINE_COLOR_CHANGED)
        glUniform3f(OutlineColorLoc, OutlineColor.R, OutlineColor.G, OutlineColor.B);
    if (UpdateState & OUTLINE_WIDTH_CHANGED)
        glUniform1f(OutlineWidthLoc, OutlineWidth);
...
void PhongShader::outlineColor(const Color& c)
{
    OutlineColor = c;
    UpdateState |= OUTLINE_COLOR_CHANGED;
}

void PhongShader::outlineWidth(float width)
{
    OutlineWidth = width;
    UpdateState |= OUTLINE_WIDTH_CHANGED;
}
